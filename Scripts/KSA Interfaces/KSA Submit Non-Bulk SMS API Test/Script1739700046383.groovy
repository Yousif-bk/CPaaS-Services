...
def authResponse = WS.sendRequest(findTestObject('KSA Interfaces OR/KSA Auth API OR'))
...
def smsResponse = WS.sendRequest(findTestObject('KSA Interfaces OR/KSA Submit Non-Bulk SMS API OR', [('access_token') : token]))

if (smsResponse.getStatusCode() == 200 || smsResponse.getStatusCode() == 201 || smsResponse.getStatusCode() == 202) {
